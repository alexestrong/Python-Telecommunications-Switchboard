...
# CHECK IF SWITCHBOARD EXISTS - DONE
...
def save_network(switchboards, file_name):
    """
    Saves the house information to a file
    :param file_name:
    :return:
    """
    '''# opens file in write mode
    new_file = open(file_name, 'w')
    # writes on each line the "device name : toggle status"
    for individual_device in self.devices:
        new_file.write(individual_device.name + ':' + str(individual_device.toggle) + '\n')
    # closes file
    new_file.close()'''
    ...
    with open(file_name, 'w') as csv_file:
        ...
        writer.writeheader()
        data_list = organize_for_save(switchboards)
        print(3, data_list)
...
def organize_for_save(switchboards):
    ...
    for keys in switchboards:
        ...
        print(0, single_line_dictionary)
        print(1, dictionary_list)
        dictionary_list.append(single_line_dictionary)
        print(2, dictionary_list)
...
def start_call(switchboards, start_area, start_number, end_area, end_number, current_area=None, checked_list=None):
    print('current area 1:', current_area)
    print('checked list 1:', checked_list)
    # print('trunk lines of current area 1:', switchboards[current_area]['trunk_lines'])
    ...
    print('current area 2:', current_area)
    print('checked list 2:', checked_list)
    print('trunk lines of current area 2:', switchboards[current_area]['trunk_lines'])
